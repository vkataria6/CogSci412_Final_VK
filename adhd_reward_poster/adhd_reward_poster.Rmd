...
## **Plan**:
1. Evaluate how reward sensitivity impacts working memory performance using response timing and accuracy metrics in 2-back tasks.
...
- **Measures**: Response times and accuracy were recorded. Incentive levels were manipulated through rewards and feedback conditions, categorized as small or large.
...
This plot shows that Neurotypical children have faster and more consistent response times under high incentive conditions, while ADHD children exhibit greater variability in their performance across both low and high incentive conditions, suggesting differences in reward sensitivity. 
...
ggplot(rt_t_test, aes(x = incentive, y = estimate, fill = incentive)) +
  ...
  labs(title = "Mean RT by Incentive (w/ Conf. Intervals)",
       x = "Incentive Condition", y = "Mean Difference in Response Time") +
  theme_minimal(base_size = 15) +
  theme(
